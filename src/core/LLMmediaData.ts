  ...
  tvResults.length > 0
    ? `电视剧选项：\n${tvResults
        ...
        .map(
          (show, idx) =>
            `TV${idx + 1}: ${show.name} (${
              show.first_air_date?.substring(0, 4) || "未知"
            })`
        )
        .join("\n")}\n`
    : ""
...
${
  movieResults.length > 0
    ? `电影选项：\n${movieResults
        ...
        .map(
          (movie, idx) =>
            `MOV${idx + 1}: ${movie.title} (${
              movie.release_date?.substring(0, 4) || "未知"
            })`
        )
        .join("\n")}\n`
    : ""
...
请只回答"类型:编号"，如"tv:1"或"movie:2"，不需要任何解释。`;
    ...
    if (match) {
      ...
      if (validIndex) {
        ...
        logger.info(
          `LLM选择: ${mediaType}, 索引: ${selectedIndex}, 标题: ${selectedName}`
        );
...
export async function searchLLMData(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      logger.info(
        `搜索结果: 电视剧=${tvResults.length}, 电影=${movieResults.length}, 集合=${collectionResults.length}`
      );

      // 使用LLM判断媒体类型
      const { mediaType, selectedData, selectedIndex } =
        await determineLLMMediaType(
          mediaInfo,
          fileName,
          tvResults,
          movieResults
      ...
      if (collectionResults.length > 0 && mediaType === "movie") {
        try {
          ...
          if (id) {
            ...
            const collectionContentID =
              collectionInfo.parts
                ?.filter(
                  (part) =>
                    part.id &&
                    movieResults.some((movie) => movie.id === part.id)
                )
      ...
      if (
        selectedIndex > 0 &&
        result.selectedData &&
        result.selectedData.length > selectedIndex
      ) {
...
export function RetrieveLLMInfo(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const processSearchResult = async (LLMData: SearchTMDBData) => {
      ...
      // 处理没有结果的情况
      if (
        !LLMData.data.selectedData ||
        LLMData.data.selectedData.length === 0
      ) {
      ...
      try {
        ...
        if (result.type === "tv") {
          if (
            ...
            episode - 1 < result.data.episodes.length
          ) {
          ...
          logger.info(
            `从文件名提取集数: ${episode}, 标题: ${result.episodeTitle}`
          );
        }
