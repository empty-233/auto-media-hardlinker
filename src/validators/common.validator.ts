...
export const CommonValidators = {
  ...
  year: z.coerce
    ...
    .max(new Date().getFullYear() + 10, "年份不能超过当前年份10年")
    .optional(),
  ...
  filePath: z
    .string()
    .min(1, "文件路径不能为空")
    .max(1024, "文件路径不能超过1024个字符"),

  /**
   * 可选的文件路径验证
   * 验证文件系统路径的合法性和长度限制，可以为空(代表根目录)
   *
   * @example
   * // 可选路径: "/anime/2024/" 或 "" (空字符串代表根目录)
   * filePath: CommonValidators.optionalFilePath
   */
  optionalFilePath: z
    .string()
    .max(1024, "文件路径不能超过1024个字符")
  ...
  mediaType: z.enum(["movie", "tv", "collection"], {
    message: "媒体类型必须是movie或tv或collection",
  }),
   ...
   */
  dateString: z.iso
    .datetime("日期格式必须是ISO 8601格式")
    .or(z.iso.date("日期格式必须是YYYY-MM-DD格式"))
    .optional(),
...
export const ParamValidators = {
  ...
  /**
   * 验证路径参数中的ID或"new"字符串
   * 用于支持创建新资源的场景，其中ID可以是数字或字符串"new"
   *
   * @example
   * // 路由: "/api/files/:id/link-media"
   * // 验证: { id: "123" } -> { id: 123 } 或 { id: "new" } -> { id: NaN }
   * router.post("/:id/link-media", ValidationMiddleware.params(ParamValidators.idOrNew), ...)
   */
  idOrNew: z.object({
    id: z.union([CommonValidators.id, z.literal("new").transform(() => NaN)]),
  }),
};
