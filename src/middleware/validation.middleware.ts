...
export function validateRequest(config: ValidationConfig) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!result.success) {
      ...
      return badRequest(res, config.customMessage || errorMessage);
    }
...
export function validateMultiple(...configs: ValidationConfig[]) {
  return configs.map((config) => validateRequest(config));
}
...
export class ValidationMiddleware {
   ...
   * 验证请求体
   */
  static body(schema: ZodType, customMessage?: string) {
   ...
   * 验证路径参数
   */
  static params(schema: ZodType, customMessage?: string) {
    return validateRequest({
      target: "params",
      schema,
      customMessage,
   ...
   * 验证查询参数
   */
  static query(schema: ZodType, customMessage?: string) {
    return validateRequest({
      target: "query",
      schema,
      customMessage,
   ...
   * 可选验证请求体（如果请求体为空则跳过）
   */
  static optionalBody(schema: ZodType, customMessage?: string) {
    return validateRequest({
      target: "body",
      schema,
      optional: true,
   ...
   */
  static optionalQuery(schema: ZodType, customMessage?: string) {
    return validateRequest({
      target: "query",
      schema,
      optional: true,
...
export class ValidatorComposer {
   ...
   * 添加请求体验证
   */
  body(schema: ZodType, customMessage?: string): ValidatorComposer {
    this.configs.push({
      target: "body",
      schema,
      customMessage,
   ...
   * 添加路径参数验证
   */
  params(schema: ZodType, customMessage?: string): ValidatorComposer {
    this.configs.push({
      target: "params",
      schema,
      customMessage,
   ...
   * 添加查询参数验证
   */
  query(schema: ZodType, customMessage?: string): ValidatorComposer {
    this.configs.push({
      target: "query",
      schema,
      customMessage,
